export async function send(text, photoFile, token, chatId) {
  ...
  const msgResponse = await fetch(sendMsgUrl, {
    ...
    body: JSON.stringify({
      chat_id: chatId,
      text: text,
      parse_mode: 'HTML' // Useful for potential formatting
    })
  });

  if (!msgResponse.ok) {
    throw new Error(`Telegram sendMessage failed: ${msgResponse.statusText}`);
  }

  const msgData = await msgResponse.json();
  
  if (!msgData.ok) {
    throw new Error(`Telegram sendMessage error: ${msgData.description}`);
  }
